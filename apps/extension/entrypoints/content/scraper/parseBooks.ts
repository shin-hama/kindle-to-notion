...
const parseNextPageState = (doc: Document): NextPageState | null => {
  const token = doc.querySelector(".kp-notebook-library-next-page-start")
    ?.getAttribute("value");
...
export const parseToDate = (
  ...
  switch (region) {
    ...
    case "france": {
      ...
      const amazonDateString = kindleDate.substring(
        kindleDate.indexOf(" ") + 1,
      );
...
export const parseBooks = (
  ...
  return Array.from(booksElm.values())
    .map((elm) => {
      const title = elm.querySelector(".kp-notebook-searchable")?.textContent ??
        "unknown";
