import { decodeBase64Url, encodeBase64Url } from "./deps.js";
...
import { decoder, encoder, isArray, isDefined, isNotNumber, isNotString, isNotTrue, isNumber, isObject, isString, isUndefined, } from "./util.js";
...
export function validateTimingClaims(payload, { expLeeway = 1, nbfLeeway = 1, ignoreExp, ignoreNbf } = {}) {
    if (hasInvalidTimingClaims(payload.exp, payload.nbf)) {
        throw new Error(`The jwt has an invalid 'exp' or 'nbf' claim.`);
    }
    if (isNumber(payload.exp) && isNotTrue(ignoreExp) &&
        isExpired(payload.exp, expLeeway)) {
        throw RangeError("The jwt is expired.");
    }
    if (isNumber(payload.nbf) && isNotTrue(ignoreNbf) &&
...
export function validateAudClaim(aud, audience) {
