...
const HighlightColors = ["yellow", "blue", "pink", "orange"];
...
const app = new Hono().post("", sessionValidator, zValidator("json", z.object({
    bookId: z.string(),
    asin: z.string(),
    ...
    try {
        ...
        const highlights = await service.createHighlights(highlightsInput.map((h) => ({
            ...h,
            bookId,
        })), c.var.user);
        ...
        for (const highlight of highlights) {
            ...
            try {
                const notionHighlight = await saveHighlight(c.var.user, bookUser.data.notionPageId, highlight.Book.asin, highlight);
